...
class RecommendFragment : BaseBindingFragment<FragmentRecommendBinding>({FragmentRecommendBinding.inflate(it)}) {
    ...
    private val progressUpdater = object : Runnable {
        override fun run() {
            if (!isSeeking && ::videoView.isInitialized) {
                ...
                val curBar = curProgressBar
                if (player != null && curBar != null) {
                    val dur = player.duration.coerceAtLeast(0)
                    if (dur > 0) curBar.updateProgress(player.currentPosition, dur)
                }
            }
    ...
    private val progressReportRunnable = object : Runnable {
        override fun run() {
            if (userVisibleHint && ::videoView.isInitialized && currentItemId.isNotEmpty() && !isSeeking) {
                val player = videoView.getplayer()
                if (player != null) {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch {
                        JellyfinManager.reportProgress(currentItemId, pos, paused)
                    }
                }
            }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            ...
            progressHandler.post(progressReportRunnable)
        } catch (e: Exception) {
            android.util.Log.e("JellyTok_CRASH", "RecommendFragment.onViewCreated CRASH: ${e.message}", e)
        }
    }
    ...
    override fun onDestroyView() {
        ...
        progressHandler.removeCallbacks(progressReportRunnable)
    ...
    private fun initRecyclerView() {
        ...
        adapter = VideoAdapter(requireContext(), rv)
        binding.recyclerView.adapter = adapter
    }

    private fun loadJellyfinVideos(append: Boolean = true) {
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val views = JellyfinManager.getViews()
                val allVideos = mutableListOf<VideoBean>()
                for (viewItem in views.items) {
                    try {
                        val resp = JellyfinManager.getVideosRecursive(viewItem.id, limit = 500)
                        for (item in resp.items) {
                            if ((item.userData?.playCount ?: 0) == 0) {
                                ...
                                val bestTicks = maxOf(serverTicks, localTicks)
                                val vb = VideoBean()
                                vb.jellyfinItemId = item.id
                                vb.jellyfinImageUrl = JellyfinManager.getImageUrl(item.id)
                                vb.videoRes = JellyfinManager.getStreamUrl(item.id)
                                vb.content = item.name
                                vb.playbackPositionTicks = bestTicks
                                vb.isLiked = item.userData?.isFavorite == true
                                vb.isLandscape = (item.width ?: 0) > (item.height ?: 0) && (item.height ?: 0) > 0
                                val ub = VideoBean.UserBean()
                                ub.nickName = "Jellyfin"
                                ub.head = R.mipmap.head1
                                vb.userBean = ub
                                allVideos.add(vb)
                            }
                        }
                ...
                allVideos.shuffle()
                if (append) adapter?.appendList(allVideos) else adapter?.setList(allVideos)
                if (allVideos.isNotEmpty() && append) {
                    binding.recyclerView.post { playCurVideo(0) }
                }
    ...
    private fun observeEvent() {
        subscribe = RxBus.getDefault().toObservable(PauseVideoEvent::class.java)
            .subscribe(Action1 { event: PauseVideoEvent ->
                if (event.isPlayOrPause) videoView.play() else videoView.pause()
    ...
    private val pageChangeCallback = object : OnPageChangeCallback() {
        override fun onPageSelected(position: Int) { playCurVideo(position) }
    }
        ...
        binding.refreshLayout.setOnRefreshListener {
            object : CountDownTimer(1000, 1000) {
                ...
                override fun onFinish() {
                    ...
                    loadJellyfinVideos(append = false)
                }
            }.start()
        }
    }
    ...
    private fun playCurVideo(position: Int) {
        ...
        tvSpeed.bringToFront()
        ivRotate.bringToFront()
        ivPlay.bringToFront()
        ...
        progressBar.setOnSeekListener(object : VideoProgressBar.OnSeekListener {
            ...
            override fun onSeekTo(positionMs: Long) {
                videoView.getplayer()?.seekTo(positionMs); videoView.play(); isSeeking = false
            }
        })
        ...
        likeView.setOnSeekListener(object : LikeView.OnSeekListener {
            ...
            override fun onSeekTo(positionMs: Long) {
                videoView.getplayer()?.seekTo(positionMs); isSeeking = false
            }
        })

        // Double-tap to toggle favorite — override adapter's animation-only handler
        likeView.setOnLikeListener(object : LikeView.OnLikeListener {
            override fun onLikeListener() {
                ...
                if (position < datas.size) {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch {
                        try {
                            JellyfinManager.toggleFavorite(item.jellyfinItemId, wasLiked)
                        } catch (_: Exception) {}
                    }
                }
            }
        ...
        likeShareEvent(controllerView, position)
        curPlayPos = position
    ...
    private fun autoPlayVideo(position: Int, ivCover: ImageView) {
        ...
        val url = datas[position].videoRes
        ...
        if (prevItemId.isNotEmpty() && prevItemId != itemId && curPlayPos >= 0 && curPlayPos < datas.size) {
            datas[curPlayPos].playbackPositionTicks = prevPos * 10_000L
            JellyfinManager.saveLocalPosition(prevItemId, prevPos)
        }
        currentItemId = itemId
        viewLifecycleOwner.lifecycleScope.launch {
            if (prevItemId.isNotEmpty() && prevItemId != itemId) {
                JellyfinManager.reportStopped(prevItemId, prevPos)
            }
    ...
    private fun likeShareEvent(controllerView: ControllerView, position: Int) {
        controllerView.setListener(object : OnVideoControllerListener {
            override fun onHeadClick() { RxBus.getDefault().post(MainPageChangeEvent(1)) }
                ...
                if (position < datas.size) {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch {
                        try {
                            ...
                            controllerView.updateLikeUI()
                        } catch (_: Exception) {}
                    }
                }
            }
            override fun onCommentClick() {
                android.widget.Toast.makeText(requireContext(), "Jellyfin 不支持评论", android.widget.Toast.LENGTH_SHORT).show()
            }
            override fun onShareClick() {
                ...
                if (position < datas.size) {
                    ...
                    val shareIntent = android.content.Intent(android.content.Intent.ACTION_SEND).apply {
                        type = "text/plain"
                        putExtra(android.content.Intent.EXTRA_TEXT, "${item.content}\n${JellyfinManager.getStreamUrl(item.jellyfinItemId)}")
                    }
                    startActivity(android.content.Intent.createChooser(shareIntent, "分享影片"))
                }
            }
        })
    }
