...
class FavoritesFeedFragment : BaseBindingFragment<FragmentRecommendBinding>({ FragmentRecommendBinding.inflate(it) }) {
    ...
    companion object {
        ...
        fun newInstance(startIndex: Int) = FavoritesFeedFragment().apply {
            arguments = Bundle().apply { putInt(ARG_START_INDEX, startIndex) }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        startIndex = arguments?.getInt(ARG_START_INDEX, 0) ?: 0
        try {
            initRecyclerView(); initVideoPlayer(); setViewPagerLayoutManager()
            loadFavorites()
            progressHandler.post(progressUpdater); progressHandler.post(progressReportRunnable)
        } catch (e: Exception) { android.util.Log.e("JellyTok_CRASH", "FavoritesFeedFragment CRASH: ${e.message}", e) }
    }
            ...
            try {
                ...
                for (item in response.items) {
                    ...
                    val vb = VideoBean().apply {
                        ...
                        playbackPositionTicks = maxOf(serverTicks, localTicks); isLiked = true
                        isLandscape = (item.width ?: 0) > (item.height ?: 0) && (item.height ?: 0) > 0
                        userBean = VideoBean.UserBean().apply { nickName = ""; head = R.mipmap.head1 }
                    }
                    allVideos.add(vb)
                }
                adapter?.setList(allVideos)
                if (allVideos.isNotEmpty()) {
                    val idx = startIndex.coerceIn(0, allVideos.size - 1)
                    binding.recyclerView.post { binding.recyclerView.setCurrentItem(idx, false); playCurVideo(idx) }
                }
